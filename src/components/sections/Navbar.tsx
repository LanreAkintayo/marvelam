...
export default function Navbar() {
    ...
    /* ── Track scroll state & active section location ─────────── */
    useEffect(() => {
        const onScroll = () => {
        ...
        <>
            ...
            >
                <nav className="mx-2 sm:mx-10 px-6 flex items-center justify-between">
                    ...
                    >
                        <span
                            className={`font-bold tracking-tight transition-all duration-300 text-slate-900 dark:text-white ${scrolled ? "text-lg" : "text-xl"
                                }`}
                        >
                            Maryam
                            ...
                            return (
                                <a
                                    ...
                                    className={`relative px-4 py-2 text-sm font-medium transition-all duration-300 rounded-lg ${isActive
                                        ? "text-amber-600 dark:text-amber-400 bg-amber-500/[0.04] dark:bg-amber-400/[0.03]"
                                ...
                                >
                                    {link.label}
                                    {/* Shared layout background pill tracking */}
                                    {isActive && (
                                        <motion.span
                                            layoutId="activeNavBackground"
                                            transition={{ type: "spring", stiffness: 380, damping: 30 }}
                                            className="absolute inset-0 rounded-lg border border-amber-500/10 dark:border-amber-400/10 pointer-events-none"
                                        />
                                    )}
                                </a>
                            );
                                ...
                                {NAV_LINKS.map((link, i) => {
                                    ...
                                    return (
                                        <motion.a
                                            ...
                                            className={`py-3.5 px-4 text-base font-medium rounded-xl transition-all duration-200 ${isActive
                                                ? "text-amber-600 dark:text-amber-400 bg-amber-500/5 border-l-2 border-amber-500 rounded-l-none pl-3.5"
                                                : "text-slate-700 dark:text-slate-200 hover:text-slate-900 dark:hover:text-white hover:bg-slate-50 dark:hover:bg-white/[0.04]"
                                                }`}
                                        >
                                            {link.label}
                                        </motion.a>
